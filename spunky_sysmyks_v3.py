...
COMMANDS = {'help': {'desc': 'display all available commands', 'syntax': '^7Usage: ^2!help', 'level': 0, 'short': 'h'},
            ...
            'glist': {'desc': 'Displays maps saved locations', 'syntax': '^7Usage: ^2!glist', 'level': 0},
            'goto': {'desc': 'Goto to a specific jump', 'syntax': '^7Usage: ^2!goto', 'level': 0},
            ...
            'votecycle': {'desc': 'Vote on cycling to the next map', 'syntax': '^7Usage: ^2!votecyclemap', 'level': 0, 'short': 'vc'},
            'votemap': {'desc': 'Callvote on changing current map', 'syntax': '^7Usage: ^2!votemap or !vm mapname',  'level': 0, 'short': 'vm'},
            'votenextmap': {'desc': 'Callvote on setting next map', 'syntax': '^7Usage: ^2!votenextmap or !vn mapname',  'level': 0, 'short': 'vn'},
            'votekick': {'desc': 'vote to kick a player', 'syntax': '^7Usage: ^2!votekick ^7<name> <reason>', 'level': 0, 'short': 'vk'},
            'votetimelimit': {'desc': 'vote to set current timelimit', 'syntax': '^7Usage: !^2votetimelimit or ^7!^2vtl ^2time.', 'level': 0, 'short': 'vtl'},
            'mapcycle': {'desc': 'lists the current map rotation', 'syntax': '^7Usage: ^2!mapcycle', 'level': 0},
            'mapname': {'desc': 'Displays current map name', 'syntax': '^7Usage: ^2!mapname', 'level': 0, 'short': 'mn'},
            'nextmap': {'desc': 'display the next map in rotation', 'syntax': '^7Usage: ^2!nextmap', 'level': 0},
            'topruns': {'desc': 'display map top run', 'syntax': '^7Usage: ^2!topruns', 'level': 0}, 
            'maprecord': {'desc': 'display map best jump run', 'syntax': '^7Usage: ^2!maprecord', 'level': 0}, 
            'record': {'desc': 'display the best run(s) of a client on a specific map', 'syntax': '^7Usage: ^2!record', 'level': 0},
            'drop': {'desc': 'Drop medkit, Helmet, Kevlar', 'syntax': '^7Usage: ^2!drop', 'level': 0},
            ...
            'setway': {'desc': 'set a name for the specified way id', 'syntax': '^7Usage: ^2!way <num> <way>', 'level': 20},
            'delrecord': {'desc': 'delrecord', 'syntax': '^7Usage: ^2!delrecord', 'level': 20},
            ...
            'walljump': {'desc': 'g_walljump 3 or 100', 'syntax': '^7Usage: ^2!walljump', 'level': 20},
            ...
            # 'forgivelist': {'desc': 'list all players who killed you', 'syntax': '^7Usage: ^2!forgivelist', 'level': 20, 'short': 'fl'},
            # 'forgiveprev': {'desc': 'forgive last team kill', 'syntax': '^7Usage: ^2!forgiveprev', 'level': 20, 'short': 'fp'},
            # 'grudge': {'desc': 'grudge a player for team killing, a grudged player will not be forgiven', 'syntax': '^7Usage: ^2!grudge ^7[<name>]', 'level': 20},
            # 'bombstats': {'desc': 'display Bomb stats', 'syntax': '^7Usage: ^2!bombstats', 'level': 20},
            # 'ctfstats': {'desc': 'display Capture the Flag stats', 'syntax': '^7Usage: ^2!ctfstats', 'level': 20},
            # 'freezestats': {'desc': 'display freeze/thawout stats', 'syntax': '^7Usage: ^2!freezestats', 'level': 20},
            # 'hestats': {'desc': 'display HE grenade kill stats', 'syntax': '^7Usage: ^2!hestats', 'level': 20},
            # 'hits': {'desc': 'display hit stats', 'syntax': '^7Usage: ^2!hits', 'level': 20},
            # 'hs': {'desc': 'display headshot counter', 'syntax': '^7Usage: ^2!hs', 'level': 20},
            # 'knife': {'desc': 'display knife kill stats', 'syntax': '^7Usage: ^2!knife', 'level': 20},
            # 'register': {'desc': 'register yourself as a basic user', 'syntax': '^7Usage: ^2!register', 'level': 20},
            # 'spree': {'desc': 'display current kill streak', 'syntax': '^7Usage: ^2!spree', 'level': 20},
            # 'stats': {'desc': 'display current map stats', 'syntax': '^7Usage: ^2!stats', 'level': 20},
            # 'teams': {'desc': 'balance teams', 'syntax': '^7Usage: ^2!teams', 'level': 20},
            # 'time': {'desc': 'display the current server time', 'syntax': '^7Usage: ^2!time', 'level': 0},
            # 'regtest': {'desc': 'display current user status', 'syntax': '^7Usage: ^2!regtest', 'level': 20},
            # 'xlrstats': {'desc': 'display full player statistics', 'syntax': '^7Usage: ^2!xlrstats ^7[<name>]', 'level': 20},
            # 'xlrtopstats': {'desc': 'display the top players', 'syntax': '^7Usage: ^2!xlrtopstats', 'level': 20, 'short': 'topstats'},
...
class LogParser(object):
    ...
    def __init__(self):
        ...
        self.tk_autokick = CONFIG.getboolean('bot', 'teamkill_autokick') if CONFIG.has_option(
            'bot', 'teamkill_autokick') else True
        self.allow_tk_bots = CONFIG.getboolean('bot', 'allow_teamkill_bots') if CONFIG.has_option(
            'bot', 'allow_teamkill_bots') else False
        # enable/disable autokick of players with low score
        self.noob_autokick = CONFIG.getboolean(
            'bot', 'noob_autokick') if CONFIG.has_option('bot', 'noob_autokick') else False
        self.spawnkill_autokick = CONFIG.getboolean(
            'bot', 'spawnkill_autokick') if CONFIG.has_option('bot', 'spawnkill_autokick') else False
        self.kill_spawnkiller = CONFIG.getboolean('bot', 'instant_kill_spawnkiller') if CONFIG.has_option(
            'bot', 'instant_kill_spawnkiller') else False
        self.spawnkill_warn_time = CONFIG.getint(
            'bot', 'spawnkill_warn_time') if CONFIG.has_option('bot', 'spawnkill_warn_time') else 3
        self.admin_immunity = CONFIG.getint(
            'bot', 'admin_immunity') if CONFIG.has_option('bot', 'admin_immunity') else 40
        # set the maximum allowed ping
        self.max_ping = CONFIG.getint('bot', 'max_ping') if CONFIG.has_option(
            'bot', 'max_ping') else 200
        # kick spectator on full server
        self.num_kick_specs = CONFIG.getint('bot', 'kick_spec_full_server') if CONFIG.has_option(
            'bot', 'kick_spec_full_server') else 10
        # set task frequency
        self.task_frequency = CONFIG.getint(
            'bot', 'task_frequency') if CONFIG.has_option('bot', 'task_frequency') else 60
        self.warn_expiration = CONFIG.getint(
            'bot', 'warn_expiration') if CONFIG.has_option('bot', 'warn_expiration') else 240
        self.bad_words_autokick = CONFIG.getint(
            'bot', 'bad_words_autokick') if CONFIG.has_option('bot', 'bad_words_autokick') else 0
        # enable/disable message 'Player connected from...'
        self.show_country_on_connect = CONFIG.getboolean(
            'bot', 'show_country_on_connect') if CONFIG.has_option('bot', 'show_country_on_connect') else True
        # enable/disable message 'Firstblood / first nade kill...'
        self.show_first_kill_msg = CONFIG.getboolean(
            'bot', 'show_first_kill') if CONFIG.has_option('bot', 'show_first_kill') else True
        self.show_hit_stats_msg = CONFIG.getboolean(
            'bot', 'show_hit_stats_respawn') if CONFIG.has_option('bot', 'show_hit_stats_respawn') else True
        self.show_multikill_msg = CONFIG.getboolean(
            'bot', 'show_multi_kill') if CONFIG.has_option('bot', 'show_multi_kill') else True
        # set teams autobalancer
        self.teams_autobalancer = CONFIG.getboolean(
            'bot', 'autobalancer') if CONFIG.has_option('bot', 'autobalancer') else False
        self.allow_cmd_teams_round_end = CONFIG.getboolean(
            'bot', 'allow_teams_round_end') if CONFIG.has_option('bot', 'allow_teams_round_end') else False
        self.limit_nextmap_votes = CONFIG.getboolean(
            'bot', 'limit_nextmap_votes') if CONFIG.has_option('bot', 'limit_nextmap_votes') else False
        self.vote_delay = CONFIG.getint(
            'bot', 'vote_delay') if CONFIG.has_option('bot', 'vote_delay') else 0
        self.spam_bomb_planted_msg = CONFIG.getboolean(
            'bot', 'spam_bomb_planted') if CONFIG.has_option('bot', 'spam_bomb_planted') else False
        self.kill_survived_opponents = CONFIG.getboolean(
            'bot', 'kill_survived_opponents') if CONFIG.has_option('bot', 'kill_survived_opponents') else False
        self.spam_knife_kills_msg = CONFIG.getboolean(
            'bot', 'spam_knife_kills') if CONFIG.has_option('bot', 'spam_knife_kills') else False
        self.spam_nade_kills_msg = CONFIG.getboolean(
            'bot', 'spam_nade_kills') if CONFIG.has_option('bot', 'spam_nade_kills') else False
        self.spam_headshot_hits_msg = CONFIG.getboolean(
            'bot', 'spam_headshot_hits') if CONFIG.has_option('bot', 'spam_headshot_hits') else False
        self.reset_headshot_hits_mapcycle = CONFIG.getboolean(
            'bot', 'reset_headshot_hits_mapcycle') if CONFIG.has_option('bot', 'reset_headshot_hits_mapcycle') else True
        self.reset_kill_spree_mapcycle = CONFIG.getboolean(
            'bot', 'reset_kill_spree_mapcycle') if CONFIG.has_option('bot', 'reset_kill_spree_mapcycle') else True
        ban_duration = CONFIG.getint('bot', 'ban_duration') if CONFIG.has_option(
            'bot', 'ban_duration') else 7
        self.ban_duration = ban_duration if ban_duration > 0 else 1
        # support for low gravity server
        self.support_lowgravity = CONFIG.getboolean('lowgrav', 'support_lowgravity') if CONFIG.has_option(
            'lowgrav', 'support_lowgravity') else False
        self.gravity = CONFIG.getint('lowgrav', 'gravity') if CONFIG.has_option(
            'lowgrav', 'gravity') else 800
        ...
        self.uptime = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
        # Rotating Messages and Rules
        if CONFIG.has_option('rules', 'show_rules') and CONFIG.getboolean('rules', 'show_rules'):
            self.output_rules = CONFIG.get('rules', 'display') if CONFIG.has_option(
                'rules', 'display') else "chat"
            rules_frequency = CONFIG.getint('rules', 'rules_frequency') if CONFIG.has_option(
                'rules', 'rules_frequency') else 90
            ...
            if os.path.isfile(self.rules_file):
                ...
                logger.error(
                    "ERROR: Rotating messages will be ignored, file '%s' has not been found", self.rules_file)
        # Parse Game log file
        try:
            # open game log file
            ...
            logger.error(
                "ERROR: The Gamelog file '%s' has not been found", games_log)
            ...
            for line in rotation_msg:
                ...
                with self.players_lock:
                    if "@admins" in line:
                        ...
                        self.game.rcon_say("^7Time: %s" % time.strftime(
                            "%H:%M", time.localtime(time.time())))
                    elif "@bigtext" in line:
                        self.game.rcon_bigtext(
                            "^7%s" % line.split('@bigtext')[-1].strip())
                    else:
    ...
    def find_game_start(self):
        ...
        try:
            ...
            logger.error(
                "ERROR: The games.log file is empty, ignoring game type and start")
            # go to the end of the file
            self.log_file.seek(0, 2)
            game_start = True
        else:
            ...
            while self.log_file:
                ...
                if len(tmp) > 1 and tmp[1] == "InitGame:":
                    ...
                    if 'g_modversion\\4.3' in line:
                        ...
                        self.hit_item.update(
                            {23: "UT_MOD_BLED", 24: "UT_MOD_KICKED", 25: "UT_MOD_KNIFE_THROWN"})
                        self.death_cause.update(
                            {42: "UT_MOD_FLAG", 43: "UT_MOD_GOOMBA"})
                        self.urt_modversion = 42
                        logger.info("Game modversion       : 4.2")
                    elif 'g_modversion\\4.1' in line:
                        # hit zone support for UrT 4.1
                        self.hit_points = {
                            0: "HEAD", 1: "HELMET", 2: "TORSO", 3: "KEVLAR", 4: "ARMS", 5: "LEGS", 6: "BODY"}
                        self.hit_item.update(
                            {21: "UT_MOD_KICKED", 22: "UT_MOD_KNIFE_THROWN"})
                    ...
                    self.default_gear = line.split('g_gear\\')[-1].split(
                        '\\')[0] if 'g_gear\\' in line else "%s" % '' if self.urt_modversion > 41 else '0'
    ...
    def read_log(self):
        ...
        if self.task_frequency > 0:
            ...
            if self.task_frequency < 10:
                ...
                schedule.every(self.task_frequency).seconds.do(
                    self.taskmanager)
        # schedule the task
    ...
    def remove_expired_db_entries(self):
        ...
        curs.execute("DELETE FROM `ban_points` WHERE `expires` < '{}'".format(
                ...
                for player in self.game.players.itervalues():
                    ...
                    if player.get_warning() > 2 and player_admin_role < 40:
                        if 'spectator' in player.get_last_warn_msg():
                            ...
                            player.add_ban_point(
                                'auto-kick for team killing', 600)
                        else:
                            kick_msg = reason = "too many warnings"
                        self.game.rcon_say("^2%s ^7was kicked, %s" %
                                           (player_name, kick_msg))
                    ...
                    if self.num_kick_specs > 0 and player_admin_role < 20:
                        ...
                        # GTV or Moderator or higher levels will not get the warning
                            ...
                            player.add_warning(
                                warning='spectator too long on full server', timer=False)
                            logger.debug(
                                "%s is spectator too long on full server", player_name)
                        ...
                        # reset spec warning
                            ...
                            player.clear_specific_warning(
                                'spectator too long on full server')
                    ...
                    if self.noob_autokick and player_admin_role < 2 and player.get_ip_address() != '0.0.0.0':
                        ...
                        ratio = round(float(kills) / float(deaths),
                                      2) if deaths > 0 else 1.0
                        ...
                        if kills > 0 and ratio < 0.33:
                            player.add_warning(
                                warning='score too low for this server', timer=False)
                            logger.debug(
                                "Score of %s is too low, ratio: %s", player_name, ratio)
                            warnmsg = "^1WARNING ^7[^3%d^7]: Your score is too low for this server" % player.get_warning(
                            )
                            self.game.rcon_tell(player_num, warnmsg, False)
                        else:
                            player.clear_specific_warning(
                                'score too low for this server')
            ...
            for player in self.game.quake.players:
                ...
                try:
                    ...
                    if self.max_ping < ping_value < 999 and gameplayer.get_admin_role() < self.admin_immunity:
                        ...
                        self.game.rcon_tell(player.num, "^1WARNING ^7[^3%d^7]: Your ping is too high [^4%d^7]. ^3The maximum allowed ping is %d." % (
                            gameplayer.get_warning(), ping_value, self.max_ping), False)
                    else:
    ...
    def handle_vote_passed(self, line):
        ...
        if "g_nextmap" in line:
            self.game.next_mapname = line.split(
                "g_nextmap")[-1].strip('"').strip()
            self.game.rcon_say(
                "^7Vote to set next map to '%s' ^2passed" % self.game.next_mapname)
        ...
        # kick vote
            ...
            self.game.rcon_say("^7Vote to kick %s ^2passed" % self.game.players[int(
                line.split('"clientkickreason "')[-1].strip('"'))].get_name())
    ...
    def handle_vote_failed(self, line):
        ...
        # nextmap vote
        if "g_nextmap" in line:
            self.game.rcon_say("^7Vote to set next map to '%s' ^1failed" % line.split(
                "g_nextmap")[-1].strip('"').strip())
            if self.vote_delay:
        ...
        # kick vote
            ...
            self.game.rcon_say("^7Vote to kick %s ^1failed" % self.game.players[int(
                line.split('"clientkickreason "')[-1].strip('"'))].get_name())
    ...
    def handle_callvote(self, line):
        ...
        if "g_nextmap" in line and self.limit_nextmap_votes and not self.allow_nextmap_vote:
            ...
            self.game.rcon_say(
                "^7Voting for Next Map is disabled until the end of this map")
            spam_msg = False
        if "map" in line and self.failed_vote_timer > now:
            remaining_time = int(self.failed_vote_timer - now)
            self.game.send_rcon('veto')
            self.game.rcon_say(
                "^7Map voting is disabled for ^2%d ^7seconds" % remaining_time)
        if spam_msg:
            self.game.rcon_bigtext("^7Press ^2F1 ^7or ^1F2 ^7to vote!")
            if self.game.get_last_maps() and ('"g_nextmap' in line or '"map' in line):
                self.game.rcon_say("^7Last Maps: ^3%s" %
                                   ", ".join(self.game.get_last_maps()))

    def new_game(self, line):
        """
        set-up a new game
        """
        self.ffa_lms_gametype = True if (
            'g_gametype\\0\\' in line or 'g_gametype\\1\\' in line or 'g_gametype\\9\\' in line or 'g_gametype\\11\\' in line) else False
        self.ctf_gametype = True if 'g_gametype\\7\\' in line else False
        self.ts_gametype = True if (
            'g_gametype\\4\\' in line or 'g_gametype\\5\\' in line) else False
    ...
    def handle_flagcapturetime(self, line):
        ...
        if action.isdigit():
            ...
            logger.debug("Player %d captured the flag in %s seconds",
                         player_num, cap_time)
            with self.players_lock:
    ...
    def stats_reset(self, store_score=False):
        ...
        with self.players_lock:
            for player in self.game.players.itervalues():
                ...
                player.reset(self.reset_headshot_hits_mapcycle,
                             self.reset_kill_spree_mapcycle)
                # reset team lock
    ...
    def handle_userinfo(self, line):
        ...
        with self.players_lock:
            ...
            if 'cl_guid' in values:
                ...
                guid = "None"
                self.kick_player_reason(
                    reason="Player with invalid GUID kicked", player_num=player_num)
            ...
            if player_num not in self.game.players:
                ...
                if player.get_ban_id():
                    self.kick_player_reason("^7%s ^1banned ^7(ID @%s): %s" % (
                        player.get_name(), player.get_ban_id(), player.get_ban_msg()), player_num)
                else:
                    if self.show_country_on_connect and player.get_country():
                        self.game.rcon_say("^7%s ^7connected from %s" % (
                            player.get_name(), player.get_country()))
            ...
            if "KEMFEW" in guid.upper():
                self.kick_player_reason(
                    "Cheater GUID detected for %s -> Player kicked" % name, player_num)
            if "WORLD" in guid.upper() or "UNKNOWN" in guid.upper():
                self.kick_player_reason(
                    "Invalid GUID detected for %s -> Player kicked" % name, player_num)

            if challenge:
                logger.debug("ClientUserinfo: Player %d %s is challenging the server and has the guid %s",
                             player_num, self.game.players[player_num].get_name(), guid)
                # kick player with hax port 1337
                invalid_port_range = ["1337"]
                if port in invalid_port_range:
                    self.kick_player_reason(
                        "Cheater Port detected for %s -> Player kicked" % name, player_num)
    ...
    def handle_userinfo_changed(self, line):
        ...
        with self.players_lock:
            ...
            if team_lock and Player.teams[team_num] != team_lock:
                ...
                self.game.rcon_tell(
                    player_num, "^3You are forced to: ^7%s" % team_lock)
            logger.debug("ClientUserinfoChanged: Player %d %s joined team %s",
                         player_num, name, Player.teams[team_num])

    def handle_begin(self, line):
        """
        handle player entering game
        """
        if self.game.get_cvar('timelimit') == '10':
        ...
        with self.players_lock:
            ...
            player_name = "%s [^5%s^7]" % (
                player_name, player_auth) if player_auth else player_name
            player_id = player.get_player_id()
            # Welcome message for registered players
            if player.get_registered_user() and player.get_welcome_msg():
                self.game.rcon_say("^3Everyone welcome back ^7%s^3, player number ^7#%s^3, to this server" % (
                    player_name, player_id))
                self.game.rcon_tell(player_num, "^7[^2Authed^7] Welcome back %s, you are ^2%s^7, last visit %s, you played %s times" % (
                    player_name, player.roles[player.get_admin_role()], player.get_last_visit(), player.get_num_played()), False)
                ...
                self.game.rcon_tell(player_num, "^7Welcome back %s, you are player ^3#%s^7." % (
                    player_name, player_id))
                player.disable_welcome_msg()
            elif player.get_first_time() and player.get_welcome_msg():
                self.game.rcon_tell(player_num, "^7Welcome %s, this must be your first visit, you are player ^3#%s^7. ^3Type ^2!help ^3in chat for help" % (
                    player_name, player_id))
                player.disable_welcome_msg()
            logger.debug(
            ...
            if hitpoint in self.hit_points:
                if self.hit_points[hitpoint] in ('HEAD', 'HELMET'):
                    ...
                    if self.spam_headshot_hits_msg and hitter_hs_count in hs_msg:
                        self.game.rcon_bigtext("^3%s: ^2%d ^7HeadShots, %s" % (
                            hitter_name, hitter_hs_count, hs_msg[hitter_hs_count]))
                    hs_plural = "headshots" if hitter_hs_count > 1 else "headshot"
                    percentage = int(
                        round(float(hitter_hs_count) / float(hitter.get_all_hits()), 2) * 100)
                    self.game.send_rcon("^7%s has ^2%d ^7%s (%d percent)" % (
                        hitter_name, hitter_hs_count, hs_plural, percentage))
                elif self.hit_points[hitpoint] in zones:
                    hitter.set_hitzones(zones[self.hit_points[hitpoint]])
                logger.debug("Player %d %s hit %d %s in the %s with %s", hitter_id, hitter_name, victim_id,
                             self.game.players[victim_id].get_name(), self.hit_points[hitpoint], self.hit_item[hit_item])
            ...
            if not self.ffa_lms_gametype:
                if victim.get_team() == killer.get_team() and victim.get_team() != 3 and victim_id != killer_id and death_cause != "UT_MOD_BOMBED":
                    ...
                    if self.firstteamkill:
                        self.game.rcon_bigtext(
                            "^1First Team Kill: ^7%s killed by ^1%s" % (victim_name, killer_name))
                        self.firstteamkill = False
                    ...
                    if killer.get_admin_role() < 2 and self.tk_autokick and killer.get_ip_address() != '0.0.0.0' and human:
                        ...
                        if killer_id not in victim.get_grudged_player():
                            ...
                            self.game.rcon_tell(
                                victim_id, "^7Type ^3!fp ^7to forgive ^3%s" % killer_name)
                        self.game.rcon_tell(
                            killer_id, "^7Do not attack teammates, you ^1killed ^7%s" % victim_name)
                        if len(killer.get_tk_victim_names()) > 3:
                            killer.ban(
                                duration=600, reason='team killing over limit', admin='bot')
                            self.game.rcon_say(
                                "^3%s ^7banned for ^110 minutes ^7for team killing over limit" % killer_name)
                            self.game.kick_player(
                                killer_id, reason='team killing over limit')
                        else:
                            killer.add_warning('stop team killing')
                            self.game.rcon_tell(
                                killer_id, "^1WARNING ^7[^3%d^7]: ^7For team killing you will get kicked" % killer.get_warning(), False)
                            if killer.get_warning() == 3 and killer.get_admin_role() < self.admin_immunity:
                                self.game.rcon_say(
                                    "^1ALERT: ^2%s ^7auto-kick from warnings if not forgiven. Type ^3!forgive %s ^7to forgive" % (killer_name, killer_id))

            suicide_reason = ['UT_MOD_SUICIDE', 'MOD_FALLING', 'MOD_WATER', 'MOD_LAVA', 'MOD_TRIGGER_HURT',
                              'UT_MOD_SPLODED', 'UT_MOD_SLAPPED', 'UT_MOD_SMITED']
            suicide_weapon = ['UT_MOD_HEGRENADE',
                              'UT_MOD_HK69', 'UT_MOD_NUKED', 'UT_MOD_BOMBED']
            # suicide counter
            if death_cause in suicide_reason or (killer_id == victim_id and death_cause in suicide_weapon):
                victim.suicide()
                victim.die()
                logger.debug("Player %d %s committed suicide with %s",
                             victim_id, victim_name, death_cause)
            # kill counter
                    ...
                    if victim.get_respawn_time() + self.spawnkill_warn_time > time.time():
                        if killer.get_ip_address() != '0.0.0.0':
                            ...
                            if self.spawnkill_autokick:
                                ...
                                self.kick_high_warns(
                                    killer, 'stop spawn killing', 'Spawn Camping and Spawn Killing are not allowed')
                        else:
                            self.game.send_rcon("smite %d" % killer_id)
                ...
                if self.show_multikill_msg:
                    if killer.get_monsterkill() == 2:
                        self.game.rcon_say(
                            "^7%s: ^2Double Kill!" % killer_name)
                    elif killer.get_monsterkill() == 3:
                        self.game.rcon_say("^7%s: ^1Multi Kill!" % killer_name)
                    elif killer.get_monsterkill() == 4:
                        self.game.rcon_say(
                            "^7%s: ^1MONSTER KILL!!" % killer_name)

                # first kill message
                if self.firstblood:
                    self.game.rcon_bigtext(
                        "^1FIRST BLOOD: ^7%s killed by ^1%s" % (victim_name, killer_name))
                    self.firstblood = False
                ...
                if death_cause == 'UT_MOD_HEGRENADE':
                    ...
                    if self.spam_nade_kills_msg and he_kill_count in event_series_msg:
                        self.game.rcon_bigtext("^3%s: ^2%d ^7HE grenade kills, %s" % (
                            killer_name, he_kill_count, event_series_msg[he_kill_count]))
                ...
                if "UT_MOD_KNIFE" in death_cause or "UT_MOD_KNIFE_THROWN" in death_cause:
                    ...
                    if self.spam_knife_kills_msg and knife_kill_count in event_series_msg:
                        self.game.rcon_bigtext("^3%s: ^2%d ^7knife kills, %s" % (
                            killer_name, knife_kill_count, event_series_msg[knife_kill_count]))
                ...
                if killer_killing_streak in kill_streak_msg and killer_id != BOT_PLAYER_NUM:
                    self.game.rcon_say("%s%s ^7%s" % (
                        killer_color, killer_name, kill_streak_msg[killer_killing_streak]))
                ...
                if victim.get_killing_streak() >= 20 and killer_name != victim_name and killer_id != BOT_PLAYER_NUM:
                    self.game.rcon_say("%s%s's ^7godlike (^1%s ^7kills) was ended by %s%s!" % (
                        victim_color, victim_name, victim.get_killing_streak(), killer_color, killer_name))
                elif victim.get_killing_streak() >= 15 and killer_name != victim_name and killer_id != BOT_PLAYER_NUM:
                    self.game.rcon_say("%s%s's ^7unstoppable (^1%s ^7kills) was ended by %s%s!" % (
                        victim_color, victim_name, victim.get_killing_streak(), killer_color, killer_name))
                elif victim.get_killing_streak() >= 10 and killer_name != victim_name and killer_id != BOT_PLAYER_NUM:
                    self.game.rcon_say("%s%s's ^7rampage (^1%s ^7kills) was ended by %s%s!" % (
                        victim_color, victim_name, victim.get_killing_streak(), killer_color, killer_name))
                elif victim.get_killing_streak() >= 5 and killer_name != victim_name and killer_id != BOT_PLAYER_NUM:
                    self.game.rcon_say("%s%s's ^7killing spree (^1%s ^7kills) was ended by %s%s!" % (
                        victim_color, victim_name, victim.get_killing_streak(), killer_color, killer_name))
                ...
                if self.show_hit_stats_msg:
                    self.game.rcon_tell(victim_id, "^1HIT Stats: ^7HS: ^2%s ^7BODY: ^2%s ^7ARMS: ^2%s ^7LEGS: ^2%s ^7TOTAL: ^2%s" % (
                        victim.get_headshots(), victim.get_hitzones('body'), victim.get_hitzones('arms'), victim.get_hitzones('legs'), victim.get_all_hits()))
                logger.debug("Player %d %s killed %d %s with %s", killer_id,
                             killer_name, victim_id, victim_name, death_cause)

    def maprandom(self):
        with open('/home/urt/news-urt/spunkybot-sysmyks/conf/settings.conf', 'r') as f:
            lines = f.readlines()

        mapcycle_start = lines.index('[mapcycle]\n')
        small_cycle_line = [line for line in lines if line.startswith('small_cycle')][0]

        # Extraire la liste de maps de small_cycle
        small_cycle = small_cycle_line.split('=')[1].strip().split(', ')

        # Mélanger la liste de maps de small_cycle
        random.shuffle(small_cycle)

        # Mettre à jour la ligne du fichier de configuration
        new_small_cycle_line = 'small_cycle = ' + ', '.join(small_cycle) + '\n'
        lines[lines.index(small_cycle_line)] = new_small_cycle_line

        # Écrire les modifications dans le fichier de configuration
        with open('/home/urt/news-urt/spunkybot-sysmyks/conf/settings.conf', 'w') as f:
            f.writelines(lines)
    ...
    def player_found(self, user):
        ...
        if not name_list:
            ...
            ret_val = False, None, "^7Players matching %s: ^3%s" % (
                user, ', '.join(name_list))
        else:
            ret_val = True, victim, "^7Found player matching %s: ^3%s" % (
                user, name_list[-1])
        return ret_val
    ...
    def offline_player(self, user_id):
        ...
        if player_id.isdigit() and int(player_id) > 1:
            curs.execute("SELECT `guid`,`name`,`ip_address` FROM `player` WHERE `id` = {}".format(
                int(player_id)))
            result = curs.fetchone()
            if result:
                victim = Player(player_num=1023, ip_address=str(
                    result[2]), guid=str(result[0]), name=str(result[1]))
    ...
    def map_found(self, map_name):
        ...
        if not map_list:
            ...
            ret_val = False, None, "^7Maps matching %s: ^3%s" % (
                map_name, ', '.join(map_list))
        else:
    ...
    def handle_say(self, line):
        ...
        handle say commands
        """
        poke_options = ['Go', 'Wake up', '*poke*',
                        'Attention', 'Get up', 'Move out']
        ...
        with self.players_lock:
            ...
            try:
                ...
                sar = {'player_num': 0, 'command': str(
                    line.split(": ", 1)).split(" ", 1)[0]}

            if sar['command'] == '!mapstats':
                self.game.rcon_tell(sar['player_num'], "^2%d ^7kills - ^2%d ^7deaths" % (
                    self.game.players[sar['player_num']].get_kills(), self.game.players[sar['player_num']].get_deaths()))
                self.game.rcon_tell(sar['player_num'], "^2%d ^7kills in a row - ^2%d ^7teamkills" % (
                    self.game.players[sar['player_num']].get_killing_streak(), self.game.players[sar['player_num']].get_team_kill_count()))
                self.game.rcon_tell(sar['player_num'], "^2%d ^7total hits - ^2%d ^7headshots" % (
                    self.game.players[sar['player_num']].get_all_hits(), self.game.players[sar['player_num']].get_headshots()))
                self.game.rcon_tell(sar['player_num'], "^2%d ^7HE grenade kills" %
                                    self.game.players[sar['player_num']].get_he_kills())
                if self.ctf_gametype:
                    if self.urt_modversion > 41:
                        self.game.rcon_tell(sar['player_num'], "^7flags captured: ^2%d ^7- flags returned: ^2%d ^7- fastest cap: ^2%s ^7sec" % (self.game.players[sar['player_num']
                                                                                                                                                                  ].get_flags_captured(), self.game.players[sar['player_num']].get_flags_returned(), self.game.players[sar['player_num']].get_flag_capture_time()))
                    else:
                        self.game.rcon_tell(sar['player_num'], "^7flags captured: ^2%d ^7- flags returned: ^2%d" % (
                            self.game.players[sar['player_num']].get_flags_captured(), self.game.players[sar['player_num']].get_flags_returned()))
                elif self.bomb_gametype:
                    self.game.rcon_tell(sar['player_num'], "^7planted: ^2%d ^7- defused: ^2%d" % (
                        self.game.players[sar['player_num']].get_planted_bomb(), self.game.players[sar['player_num']].get_defused_bomb()))
                    self.game.rcon_tell(sar['player_num'], "^7bomb carrier killed: ^2%d ^7- enemies bombed: ^2%d" % (
                        self.game.players[sar['player_num']].get_bomb_carrier_kills(), self.game.players[sar['player_num']].get_kills_with_bomb()))
                elif self.freeze_gametype:
                    self.game.rcon_tell(sar['player_num'], "^7freeze: ^2%d ^7- thaw out: ^2%d" % (
                        self.game.players[sar['player_num']].get_freeze(), self.game.players[sar['player_num']].get_thawout()))
...
# jumper commands
                ...
                self.game.rcon_tell(sar['player_num'], "Drop medkit and Helmet")
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.send_rcon("spoof %s goto %d" % (
                            self.game.players[sar['player_num']].get_name(), victim.get_player_num()))
                else:
                    self.game.rcon_tell(sar['player_num'],
                                        "^7Usage: [^2!tp^7] [^2playername^7] ^7to teleport to their location.")
            ...
            #!goto
                ...
                if (jumpName != ""):
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], "^7Usage: [^2!glist^7] ^7to see available jump locations, then [!^2goto jumpname^7]")
            ...
            elif sar['command'] == '!vote':
                self.game.rcon_tell(
                    sar['player_num'], "^7[!^2votecylemap^7/!^2vc^7]: Calls a vote to cycle the current map.")
                self.game.rcon_tell(
                    sar['player_num'], "^7[!^2votekick^7/!^2vk^7]: Calls a vote to kick a player.")
                self.game.rcon_tell(
                    sar['player_num'], "^7[!^2votemap^7/!^2vm^7]: Calls a vote to change current map.")
                self.game.rcon_tell(
                    sar['player_num'], "^7[!^2votenextmap^7/!^2vn^7]: Calls a vote to change nextmap.")
                self.game.rcon_tell(
                    sar['player_num'], "^7[!^2votetimelimit^7/!^2vtl^7]: Calls a vote to change timelimit.")
                self.game.rcon_tell(
                    sar['player_num'], "^7Check console: to view all [^2!vote calls^7] options by pressing [^2`^7] key.")
            ...
            elif sar['command'] in ('!votetimelimit', '!vtl'):
                player_num = sar['player_num']
                if line.split(sar['command'])[1]:
                    timelimit = line.split(sar['command'])[1].strip()
                    if self.game.get_cvar('timelimit') == timelimit:
                        self.game.rcon_tell(
                            sar['player_num'], "^7[^2Timelimit^7]: is already set to: [^2%s^7] minutes." % timelimit)
                    else:
                        self.game.send_rcon(
                            "spoof %s callvote timelimit %s" % (player_num, timelimit))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['votetimelimit']['syntax'])
                    return

            # allowgot
            elif sar['command'] in ('!allowgoto', '!alg'):
                self.game.send_rcon("spoof %s allowgoto" % (
                    self.game.players[sar['player_num']].get_name()))

            # !transl <lang> <text>
            elif sar['command'] in ('!transl', '!tr') and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['transl']['level']:
                ...
                with self.players_lock:
                    for player in self.game.players.itervalues():
                        ...
                        if player.get_team() == 3:
                            ...
                            self.game.send_rcon("spoof %s ready" % (
                                self.game.players[sar['player_num']].get_name()))
                            return
                ...
                with self.players_lock:
                    for player in self.game.players.itervalues():
                        ...
                        if player.get_team() == 3:
                            ...
                            return
            # timelimit
            elif sar['command'] == '!timelimit' and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['map']['level']:
                player_num = sar['player_num']
                if line.split(sar['command'])[1]:
                    timelimit = line.split(sar['command'])[1].strip()
                    if self.game.get_cvar('timelimit') == timelimit:
                        ...
                        self.game.send_rcon("timelimit %s" % (timelimit))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7Usage: !timelimit #.")
            ...
            elif sar['command'] in ('!topruns'):
                print("")
                
            elif sar['command'] in ('!setway') and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['download']['level']:
                print("")
            
            elif sar['command'] in ('!delrecord') and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['download']['level']:
                print("")
                
            elif sar['command'] in ('!maprecord') and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['download']['level']:
                print("")  

            elif sar['command'] in ('!record'):
                print("")             
...
# player moderator
                ...
                if line.split(sar['command'])[1]:
                    ...
                    for map in maps:
                        ...
                        if map_already_exist:
                            try:
                                full_url = CONFIG.get(
                                    'server', 'download_url')+map_name+".pk3"
                                print(full_url)
                                self.game.rcon_say(
                                    "^7Downloading map: [^2%s^7], server will reload once complete!" % map)
                                wget.download(full_url, out=CONFIG.get(
                                    'server', 'download_folder'))
                                map_list.append(map)
                                self.game.rcon_say(
                                    "^7Downloading map: [^2%s^7] (Finished!), server will now reload!" % map)
                                time.sleep(2)
                                reload = True
                            except:
                                self.game.rcon_tell(
                                    sar['player_num'], "^7Can't download the map: ^3%s in Fliros UrT maps server address!" % map)
                                continue
                        else:
                            self.game.rcon_tell(sar['player_num'], "^7Map: [^2%s^7] is already on server!" % map)
            ...
            # register - register yourself as a basic user
                ...
                if not self.game.players[sar['player_num']].get_registered_user():
                    self.game.players[sar['player_num']
                                      ].register_user_db(role=1)
                    self.game.rcon_tell(sar['player_num'], "^3%s ^7put in group User" %
                                        self.game.players[sar['player_num']].get_name())
                else:
                    self.game.rcon_tell(sar['player_num'], "^3%s ^7is already in a higher level group" %
                                        self.game.players[sar['player_num']].get_name())

            # regtest - display current user status
            elif sar['command'] == '!regtest':
                if self.game.players[sar['player_num']].get_registered_user():
                    self.game.rcon_tell(sar['player_num'], "^7%s [^3@%s^7] is registered since ^3%s" % (self.game.players[sar['player_num']].get_name(
                    ), self.game.players[sar['player_num']].get_player_id(), self.game.players[sar['player_num']].get_first_seen_date()))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7You are not a registered user.")

            # hs - display headshot counter
            elif sar['command'] == '!hs':
                hs_count = self.game.players[sar['player_num']].get_headshots()
                if hs_count > 0:
                    self.game.rcon_tell(sar['player_num'], "^7You made ^2%d ^7headshot%s" % (
                        hs_count, 's' if hs_count > 1 else ''))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7You made no headshot")
            ...
            # spree - display kill streak counter
                ...
                if spree_count > 0:
                    self.game.rcon_tell(sar['player_num'], "^7You have ^2%d ^7kill%s in a row" % (
                        spree_count, 's' if spree_count > 1 else ''))
                elif lose_count > 1:
                    self.game.rcon_tell(
                        sar['player_num'], "^7You have a losing spree with ^1%d ^7deaths in a row" % lose_count)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7You are currently not having a killing spree")
            ...
            # hestats - display HE grenade kill counter
                ...
                if he_kill_count > 0:
                    self.game.rcon_tell(sar['player_num'], "^7You made ^2%d ^7HE grenade kill%s" % (
                        he_kill_count, 's' if he_kill_count > 1 else ''))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7You made no HE grenade kill")
            ...
            # knife - display knife kill counter
                ...
                if knife_kill_count > 0:
                    self.game.rcon_tell(sar['player_num'], "^7You made ^2%d ^7knife kill%s" % (
                        knife_kill_count, 's' if knife_kill_count > 1 else ''))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7You made no knife kill")

            # hits - display hit stats
            elif sar['command'] == '!hits':
                self.game.rcon_tell(sar['player_num'], "^1HIT Stats: ^7HS: ^2%s ^7BODY: ^2%s ^7ARMS: ^2%s ^7LEGS: ^2%s ^7TOTAL: ^2%s" % (self.game.players[sar['player_num']].get_headshots(), self.game.players[sar['player_num']].get_hitzones(
                    'body'), self.game.players[sar['player_num']].get_hitzones('arms'), self.game.players[sar['player_num']].get_hitzones('legs'), self.game.players[sar['player_num']].get_all_hits()))
            ...
            # bombstats - display bomb statistics
                ...
                if self.bomb_gametype:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], "^7You are not playing Bomb")
            ...
            # ctfstats - display ctf statistics
                ...
                if self.ctf_gametype:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], "^7You are not playing Capture the Flag")
            ...
            # freezestats - display freeze tag statistics
                ...
                if self.freeze_gametype:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], "^7You are not playing Freeze Tag")

            # time - display the servers current time
            elif sar['command'] in ('!time', '@time'):
                msg = "^7%s" % time.strftime(
                    "%H:%M", time.localtime(time.time()))
                self.tell_say_message(sar, msg)
            ...
            # stats - display current map stats
                ...
                if not self.freeze_gametype:
                    ratio = round(float(self.game.players[sar['player_num']].get_kills()) / float(
                        self.game.players[sar['player_num']].get_deaths()), 2) if self.game.players[sar['player_num']].get_deaths() > 0 else 1.0
                    self.game.rcon_tell(sar['player_num'], "^7Map Stats %s: ^7K ^2%d ^7D ^3%d ^7TK ^1%d ^7Ratio ^5%s ^7HS ^2%d" % (self.game.players[sar['player_num']].get_name(), self.game.players[sar['player_num']].get_kills(
                    ), self.game.players[sar['player_num']].get_deaths(), self.game.players[sar['player_num']].get_team_kill_count(), ratio, self.game.players[sar['player_num']].get_headshots()))
                else:
                    # Freeze Tag
                    self.game.rcon_tell(sar['player_num'], "^7Freeze Stats %s: ^7F ^2%d ^7T ^3%d ^7TK ^1%d ^7HS ^2%d" % (self.game.players[sar['player_num']].get_name(), self.game.players[sar['player_num']].get_freeze(
                    ), self.game.players[sar['player_num']].get_thawout(), self.game.players[sar['player_num']].get_team_kill_count(), self.game.players[sar['player_num']].get_headshots()))
                    ...
                    for player in self.game.players.itervalues():
                        if (arg.upper() in (player.get_name()).upper()) or (arg == str(player.get_player_num())) or (arg == ("@%s" % player.get_player_id())) or (arg.lower() == player.get_authname()):
                            ...
                            if player.get_registered_user():
                                ratio = round(float(player.get_db_kills(
                                )) / float(player.get_db_deaths()), 2) if player.get_db_deaths() > 0 else 1.0
                                self.game.rcon_tell(sar['player_num'], "^7Stats %s: ^7K ^2%d ^7D ^3%d ^7TK ^1%d ^7Ratio ^5%s ^7HS ^2%d" % (
                                    player.get_name(), player.get_db_kills(), player.get_db_deaths(), player.get_db_tks(), ratio, player.get_db_headshots()))
                            else:
                                self.game.rcon_tell(
                                    sar['player_num'], "^7Sorry, this player is not registered")
                            break
                    if not player_found:
                        self.game.rcon_tell(
                            sar['player_num'], "^7No player found matching ^3%s" % arg)
                else:
                    if self.game.players[sar['player_num']].get_registered_user():
                        ratio = round(float(self.game.players[sar['player_num']].get_db_kills()) / float(
                            self.game.players[sar['player_num']].get_db_deaths()), 2) if self.game.players[sar['player_num']].get_db_deaths() > 0 else 1.0
                        self.game.rcon_tell(sar['player_num'], "^7Stats %s: ^7K ^2%d ^7D ^3%d ^7TK ^1%d ^7Ratio ^5%s ^7HS ^2%d" % (self.game.players[sar['player_num']].get_name(), self.game.players[sar['player_num']].get_db_kills(
                        ), self.game.players[sar['player_num']].get_db_deaths(), self.game.players[sar['player_num']].get_db_tks(), ratio, self.game.players[sar['player_num']].get_db_headshots()))
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], "^7You need to ^2!register ^7first")

            # xlrtopstats
            elif (sar['command'] in ('!xlrtopstats', '!topstats')) and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['xlrtopstats']['level']:
                curs.execute("SELECT name FROM `xlrstats` WHERE (`rounds` > 35 or `kills` > 500) and `last_played` > '{}' ORDER BY `ratio` DESC LIMIT 3".format(
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime((time.time() - 10368000)))))  # last played within the last 120 days
                result = curs.fetchall()
                toplist = ['^1#%s ^7%s' % (index + 1, result[index][0])
                           for index in xrange(len(result))]
                msg = "^3Top players: %s" % str(
                    ", ".join(toplist)) if toplist else "^3Awards still available"
                self.game.rcon_tell(sar['player_num'], msg)
                ...
                if victim.get_killed_me():
                    if line.split(sar['command'])[1]:
                        ...
                        if not found:
                            ...
                            forgive_player_num = forgive_player.get_player_num(
                            ) if forgive_player.get_player_num() in victim.get_killed_me() else False
                    else:
                    ...
                    if forgive_player_num is not False:
                        ...
                        self.game.rcon_say("^7%s has forgiven %s's attack" % (
                            victim.get_name(), forgive_player.get_name()))
                    else:
                        self.game.rcon_tell(sar['player_num'], "^7Whom to forgive? %s" % ", ".join(["^3%s [^2%s^3]" % (
                            self.game.players[playernum].get_name(), playernum) for playernum in list(set(victim.get_killed_me()))]))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^3No one to forgive")
            ...
            # forgive all team kills
                ...
                if msg:
                    self.game.rcon_say("^7%s has forgiven: ^3%s" % (
                        victim.get_name(), ", ".join(msg)))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^3No one to forgive")
            ...
            # grudge - grudge a player for team killing (a grudged player will not be forgiven) - !grudge [<name>]
                ...
                if victim.get_killed_me():
                    if line.split(sar['command'])[1]:
                        ...
                        if not found:
                            self.game.rcon_tell(sar['player_num'], "^7Whom to grudge? %s" % ", ".join(["^3%s [^2%s^3]" % (
                                self.game.players[playernum].get_name(), playernum) for playernum in list(set(victim.get_killed_me()))]))
                        else:
                            victim.set_grudge(grudge_player.get_player_num())
                            self.game.rcon_say("^7%s has a grudge against ^1%s" % (
                                victim.get_name(), grudge_player.get_name()))
                    else:
                        grudge_player = self.game.players[victim.get_killed_me(
                        )[-1]]
                        victim.set_grudge(grudge_player.get_player_num())
                        self.game.rcon_say("^7%s has a grudge against ^1%s" % (
                            victim.get_name(), grudge_player.get_name()))
                elif victim.get_grudged_player():
                    self.game.rcon_tell(sar['player_num'], "^7No one to grudge. You already have a grudge against: %s" % ", ".join(
                        ["^3%s" % self.game.players[playernum].get_name() for playernum in victim.get_grudged_player()]))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^3No one to grudge")
...
# mod level 20
            # admintest - display current admin status
                ...
                self.game.rcon_tell(sar['player_num'], "^7%s [^3@%s^7] is ^3%s ^7[^2%d^7]" % (self.game.players[sar['player_num']].get_name(
                ), self.game.players[sar['player_num']].get_player_id(), self.game.players[sar['player_num']].roles[player_admin_role], player_admin_role))
            ...
            # country / locate
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        msg = "^3%s ^7is connecting from ^3%s" % (
                            victim.get_name(), victim.get_country())
                        self.tell_say_message(sar, msg)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['country']['syntax'])
            ...
            # poke - notify a player that he needs to move
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.rcon_tell(sar['player_num'], "^7%s %s!" % (
                            random.choice(poke_options), victim.get_name()))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['poke']['syntax'])
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if victim_admin_role > 0:
                            self.game.rcon_tell(sar['player_num'], "^7%s [^3@%s^7] is ^3%s ^7[^2%d^7] and registered since ^3%s" % (
                                victim.get_name(), victim.get_player_id(), victim.roles[victim_admin_role], victim_admin_role, victim.get_first_seen_date()))
                        else:
                            self.game.rcon_tell(sar['player_num'], "^7%s [^3@%s^7] is ^3%s ^7[^2%d^7]" % (
                                victim.get_name(), victim.get_player_id(), victim.roles[victim_admin_role], victim_admin_role))
                else:
                    self.game.rcon_tell(sar['player_num'], "^3Level %s [^2%d^3]: ^7%s" % (self.game.players[sar['player_num']].get_name(
                    ), self.game.players[sar['player_num']].get_admin_role(), self.game.players[sar['player_num']].roles[self.game.players[sar['player_num']].get_admin_role()]))
            ...
            # lastmaps - list the last played maps
                ...
                if self.game.get_last_maps():
                    self.game.rcon_tell(sar['player_num'], "^7Last Maps: ^3%s" % ", ".join(
                        self.game.get_last_maps()))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7No maps have been played since Spunky Bot started")
            ...
            # lastvote - display information about the last called vote
                ...
                if self.last_vote:
                    self.game.rcon_tell(
                        sar['player_num'], "^7Last vote: ^2%s" % self.last_vote)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7No votes have been called since Spunky Bot started")

            # list - list all connected players
            elif sar['command'] == '!list' and self.game.players[sar['player_num']].get_admin_role() >= COMMANDS['list']['level']:
                msg = "^7Players online: %s" % ", ".join(["^3%s [^2%d^3]" % (player.get_name(), player.get_player_num(
                )) for player in self.game.players.itervalues() if player.get_player_num() != BOT_PLAYER_NUM])
                self.game.rcon_tell(sar['player_num'], msg)
            ...
            # mute - mute or unmute a player
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.send_rcon("mute %d %s" % (
                            victim.get_player_num(), duration))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['mute']['syntax'])
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if victim.get_registered_user():
                            self.game.rcon_tell(sar['player_num'], "^3%s ^7was last seen on %s" % (
                                victim.get_name(), victim.get_last_visit()))
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3%s ^7is not a registered user" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['seen']['syntax'])
            ...
            # shuffleteams
                ...
                if not self.ffa_lms_gametype:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], "^7Command is disabled for this game mode")
            ...
            # warninfo - display how many warnings the player has
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        warn_time = int(math.ceil(
                            float(victim.get_last_warn_time() + self.warn_expiration - time.time()) / 60))
                        self.game.rcon_tell(sar['player_num'], "^3%s ^7has ^2%s ^7active warning%s%s" % (victim.get_name(), warn_count if warn_count > 0 else 'no', 's' if warn_count >
                                            1 else '', ", expires in ^1%s ^7minute%s: ^3%s" % (warn_time, "s" if warn_time > 1 else "", ", ^3".join(victim.get_all_warn_msg())) if warn_count > 0 else ''))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['warninfo']['syntax'])
            ...
            # warn - warn user - !warn <name> [<reason>]
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if arg:
                        user = arg[0]
                        reason = ' '.join(arg[1:])[:40].strip() if len(
                            arg) > 1 else 'behave yourself'
                        found, victim, msg = self.player_found(user)
                        if not found:
                            self.game.rcon_tell(sar['player_num'], msg)
                        else:
                            ...
                            if victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                                self.game.rcon_tell(
                                    sar['player_num'], "^3You cannot warn an admin")
                            elif victim.get_last_warn_time() + warn_delay > time.time():
                                self.game.rcon_tell(
                                    sar['player_num'], "^3Only one warning per %d seconds can be issued" % warn_delay)
                            else:
                                # clear if already expired
                                if victim.get_last_warn_time() + self.warn_expiration < time.time():
                                    victim.clear_warning()
                                ...
                                if victim.get_warning() > 2:
                                    self.game.kick_player(
                                        victim.get_player_num(), reason='too many warnings')
                                    msg = "^2%s ^7was kicked, too many warnings" % victim.get_name()
                                else:
                                    if reason in REASONS:
                                        warning = REASONS[reason]
                                        if reason == 'tk' and victim.get_warning() > 1:
                                            ban_duration = victim.add_ban_point(
                                                'tk, ban by %s' % self.game.players[sar['player_num']].get_name(), 900)
                                    ...
                                    msg = "^1WARNING ^7[^3%d^7]: ^2%s^7: %s" % (
                                        victim.get_warning(), victim.get_name(), warning)
                                    # ban player if needed
                                    if ban_duration > 0:
                                        msg = "^2%s ^7banned for ^1%d minutes ^7for too many warnings" % (
                                            victim.get_name(), ban_duration)
                                        self.game.kick_player(
                                            victim.get_player_num(), reason='too many warnings')
                                ...
                                if show_alert:
                                    self.game.rcon_say(
                                        "^1ALERT: ^2%s ^7auto-kick from warnings if not cleared" % victim.get_name())
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['warn']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['warn']['syntax'])
            ...
            # warnremove - remove a player's last warning
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if last_warning:
                            self.game.rcon_say("^7Last warning removed for %s: ^3%s" % (
                                victim.get_name(), last_warning))
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3%s ^7has no active warning" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['warnremove']['syntax'])
            ...
            # warns - list the warnings
                ...
                keylist.sort()
                self.game.rcon_tell(
                    sar['player_num'], "^7Warnings: ^3%s" % ", ^3".join([key for key in keylist]))
            ...
            # warntest - test a warning
                ...
                self.game.rcon_tell(
                    sar['player_num'], "^2TEST: ^1WARNING ^7[^31^7]: ^4%s" % warning)
...
# admin level 40
            ...
            # !regulars/!regs - display the regular players online
                ...
                liste = "%s" % ", ".join(["^3%s [^2%d^3]" % (player.get_name(), player.get_admin_role(
                )) for player in self.game.players.itervalues() if player.get_admin_role() == 2])
            ...
            # aliases - list the aliases of the player
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        msg = "^7Aliases of ^5%s: ^3%s" % (
                            victim.get_name(), victim.get_aliases())
                        self.tell_say_message(sar, msg)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['aliases']['syntax'])
            ...
            # bigtext - display big message on screen
                ...
                if line.split(sar['command'])[1]:
                    self.game.rcon_bigtext(
                        "%s" % line.split(sar['command'])[1].strip())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['bigtext']['syntax'])
            ...
            # say - say a message to all players
                ...
                if line.split(sar['command'])[1]:
                    self.game.rcon_say("^4%s: ^7%s" % (self.game.players[sar['player_num']].get_name(
                    ), line.split(sar['command'])[1].strip()))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['say']['syntax'])
            ...
            # !!<text> - allow spectator to say a message to players in-game
                ...
                if line.split('!!')[1]:
                    self.game.rcon_say("^4%s: ^7%s" % (
                        self.game.players[sar['player_num']].get_name(), line.split('!!', 1)[1].strip()))
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if len(arg) > 1:
                        ...
                        if not found:
                            ...
                            self.game.rcon_tell(victim.get_player_num(), "^4%s: ^7%s" % (
                                self.game.players[sar['player_num']].get_name(), message))
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['tell']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['tell']['syntax'])
            ...
            # exit - display last disconnected player of this match
                ...
                msg = "^7Last disconnected player: ^3%s" % self.last_disconnected_player.get_name(
                ) if self.last_disconnected_player else "^3No player left during this match"
                self.game.rcon_tell(sar['player_num'], msg)
            ...
            # find - display the slot number of the player
                ...
                if line.split(sar['command'])[1]:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['find']['syntax'])
            ...
            # afk - force a player to spec, because he is away from keyboard - !afk <name>
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.rcon_forceteam(
                            victim.get_player_num(), 'spectator')
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['afk']['syntax'])
                    ...
                    if len(arg) > 1:
                        ...
                        if not found:
                            ...
                            if team in team_dict:
                                ...
                                self.game.rcon_forceteam(
                                    victim_player_num, team_dict[team])
                                self.game.rcon_tell(
                                    victim_player_num, "^3You are forced to: ^7%s" % team_dict[team])
                            ...
                            # release the player from a forced team
                                ...
                                self.game.rcon_tell(
                                    sar['player_num'], COMMANDS['force']['syntax'])
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['force']['syntax'])
                else:
            ...
            # nuke - nuke a player
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                            self.game.rcon_tell(
                                sar['player_num'], "^3Insufficient privileges to nuke an admin")
                        else:
                            self.game.send_rcon("nuke %d" %
                                                victim.get_player_num())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['nuke']['syntax'])
                    ...
                    if user and reason:
                        ...
                        if not found:
                            ...
                            if sar['player_num'] == victim.get_player_num():
                                self.game.rcon_tell(
                                    sar['player_num'], "^7You cannot kick yourself")
                            elif victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                                self.game.rcon_tell(
                                    sar['player_num'], "^3Insufficient privileges to kick an admin")
                            else:
                                msg = "^2%s ^7was kicked by %s" % (
                                    victim.get_name(), self.game.players[sar['player_num']].get_name())
                                if reason in REASONS:
                                    ...
                                    msg = "%s: ^3%s" % (msg, kick_reason)
                                self.game.kick_player(
                                    victim.get_player_num(), reason=kick_reason)
                                self.game.rcon_say(msg)
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], "^7You need to enter a reason: ^3!kick <name> <reason>")
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['kick']['syntax'])
            ...
            # warnclear - clear the user warnings
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.rcon_say(
                            "^1All warnings and team kills cleared for ^2%s" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['warnclear']['syntax'])
            ...
            # tempban - ban a player temporary for the given period
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if len(arg) > 1:
                        ...
                        reason = ' '.join(arg[2:])[:40].strip() if len(
                            arg) > 2 else 'tempban'
                        ...
                        if not found:
                            ...
                            if sar['player_num'] == victim.get_player_num():
                                self.game.rcon_tell(
                                    sar['player_num'], "^7You cannot ban yourself")
                            elif victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                                self.game.rcon_tell(
                                    sar['player_num'], "^3Insufficient privileges to ban an admin")
                            else:
                                if victim.ban(duration=duration, reason=reason, admin=self.game.players[sar['player_num']].get_name()):
                                    msg = "^2%s ^1banned ^7for ^3%s ^7by %s" % (victim.get_name(
                                    ), duration_output, self.game.players[sar['player_num']].get_name())
                                    if kick_reason:
                                        msg = "%s: ^3%s" % (msg, kick_reason)
                                    self.game.rcon_say(msg)
                                else:
                                    self.game.rcon_tell(
                                        sar['player_num'], "^7This player has already a longer ban")
                                self.game.kick_player(
                                    player_num=victim.get_player_num(), reason=kick_reason)
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], "^7You need to enter a duration: ^3!tempban <name> <duration> [<reason>]")
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['tempban']['syntax'])
...
# full admin level 60
            # !forgiveinfo <name> - display a player's team kills
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.rcon_tell(sar['player_num'], "^3%s ^7killed ^1%s ^7teammate%s" % (victim.get_name(
                        ), tks, 's' if tks > 1 else '') if tks > 0 else "^3%s ^7has not killed teammates" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['forgiveinfo']['syntax'])
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        for player in self.game.players.itervalues():
                            player.clear_tk(victim.get_player_num())
                        self.game.rcon_say(
                            "^1All team kills cleared for ^2%s" % victim.get_name())
                else:
                    for player in self.game.players.itervalues():
                        player.clear_all_tk()
            ...
            # ping - display the ping of a player
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        msg = "^7%s has a ping of ^2%s ms" % (
                            victim.get_name(), player_ping)
                        self.game.rcon_tell(sar['player_num'], msg)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['ping']['syntax'])
            ...
            # id - show the IP, guid and authname of a player - !id <name>
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        msg = "^7[^2@%s^7] %s ^2%s ^7[^5%s^7] since ^3%s" % (victim.get_player_id(), victim.get_name(
                        ), victim.get_ip_address(), victim.get_authname() if victim.get_authname() else "---", victim.get_first_seen_date())
                        self.game.rcon_tell(sar['player_num'], msg)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['id']['syntax'])
            ...
            # scream - scream a message in different colors to all players
                ...
                if line.split(sar['command'])[1]:
                    self.game.rcon_say("^1%s" % line.split(
                        sar['command'])[1].strip())
                    self.game.rcon_say("^2%s" % line.split(
                        sar['command'])[1].strip())
                    self.game.rcon_say("^3%s" % line.split(
                        sar['command'])[1].strip())
                    self.game.rcon_say("^5%s" % line.split(
                        sar['command'])[1].strip())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['scream']['syntax'])
            ...
            # slap - slap a player (a number of times); (1-15 times)
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                            self.game.rcon_tell(
                                sar['player_num'], "^3Insufficient privileges to slap an admin")
                        else:
                            for _ in xrange(0, number):
                                self.game.send_rcon(
                                    "slap %d" % victim.get_player_num())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['slap']['syntax'])
                ...
                if not self.ffa_lms_gametype:
                    if line.split(sar['command'])[1]:
                        ...
                        if len(arg) >= 1:
                            ...
                            found2, victim2, _ = (True, self.game.players[sar['player_num']], "") if len(
                                arg) == 1 else self.player_found(arg[1])
                            if not found1 or not found2:
                                self.game.rcon_tell(
                                    sar['player_num'], '^3Player not found')
                            else:
                                team1 = victim1.get_team()
                                team2 = victim2.get_team()
                                if team1 == team2:
                                    self.game.rcon_tell(
                                        sar['player_num'], "^7Cannot swap, both players are in the same team")
                                else:
                                    ...
                                    if game_data[Player.teams[team1]] < game_data[Player.teams[team2]]:
                                        self.game.rcon_forceteam(
                                            victim2.get_player_num(), Player.teams[team1])
                                        self.game.rcon_forceteam(
                                            victim1.get_player_num(), Player.teams[team2])
                                    else:
                                        self.game.rcon_forceteam(
                                            victim1.get_player_num(), Player.teams[team2])
                                        self.game.rcon_forceteam(
                                            victim2.get_player_num(), Player.teams[team1])
                                    self.game.rcon_say('^7Swapped player ^3%s ^7with ^3%s' % (
                                        victim1.get_name(), victim2.get_name()))
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], COMMANDS['swap']['syntax'])
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['swap']['syntax'])
                else:
            ...
            # status - report the status of the bot
                ...
                msg = "^7Database is ^2UP^7 and Bot started at ^2%s" % self.uptime if curs.fetchall(
                ) else "^7Database appears to be ^1DOWN"
                self.game.rcon_tell(sar['player_num'], msg)
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if player_ping == 999:
                            ...
                            ), reason='connection interrupted, try to reconnect')
                            self.game.rcon_say("^2%s ^7was kicked by %s: ^4connection interrupted" % (
                                victim.get_name(), self.game.players[sar['player_num']].get_name()))
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3%s has no connection interrupted" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['ci']['syntax'])
            ...
            # ban - ban a player for several days
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if user and reason:
                        ...
                        if not found:
                            ...
                            if sar['player_num'] == victim.get_player_num():
                                ...
                                if victim.ban(duration=(self.ban_duration * 86400), reason=reason, admin=self.game.players[sar['player_num']].get_name()):
                                    msg = "^2%s ^1banned ^7for ^3%d day%s ^7by %s" % (victim.get_name(
                                    ), self.ban_duration, 's' if self.ban_duration > 1 else '', self.game.players[sar['player_num']].get_name())
                                    if kick_reason:
                                        msg = "%s: ^3%s" % (msg, kick_reason)
                                    self.game.rcon_say(msg)
                                else:
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], "^7You need to enter a reason: ^3!ban <name> <reason>")
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['ban']['syntax'])
            ...
            # baninfo - display active bans of a player
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        curs.execute("SELECT `expires` FROM `ban_list` WHERE `expires` > '{}' AND `guid` = '{}'".format(
                            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time())), victim.get_guid()))
                        result = curs.fetchone()
                        if result:
                            self.game.rcon_tell(sar['player_num'], "^3%s ^7has an active ban until [^1%s^7]" % (
                                victim.get_name(), str(result[0])))
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3%s ^7has no active ban" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['baninfo']['syntax'])
            ...
            # rain - enables or disables rain - !rain <on/off>
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if arg == "off":
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['rain']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['rain']['syntax'])
...
# senior admin level 80
            # !kickall <pattern> [<reason>]- kick all players matching <pattern>
                ...
                if line.split(sar['command'])[1]:
                    ...
                    user = arg[0]
                    reason = ' '.join(arg[1:])[
                        :40].strip() if len(arg) > 1 else ''
                    if len(user) > 2:
                        pattern_list = [player for player in self.game.players.itervalues() if user.upper(
                        ) in player.get_name().upper() and player.get_player_num() != BOT_PLAYER_NUM]
                        if pattern_list:
                            for player in pattern_list:
                                if player.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                                    ...
                                    self.game.kick_player(
                                        player.get_player_num(), reason)
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3No Players found matching %s" % user)
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], "^3Pattern must be at least 3 characters long")
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['kickall']['syntax'])
            ...
            # !banall <pattern> [<reason>]- ban all players matching <pattern>
                ...
                if line.split(sar['command'])[1]:
                        ...
                        if pattern_list:
                            for player in pattern_list:
                                if player.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                                    ...
                                    player.ban(duration=(self.ban_duration * 86400), reason=reason,
                                               admin=self.game.players[sar['player_num']].get_name())
                                    self.game.rcon_say("^2%s ^1banned ^7for ^3%d day%s ^7by %s" % (player.get_name(
                                    ), self.ban_duration, 's' if self.ban_duration > 1 else '', self.game.players[sar['player_num']].get_name()))
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3No Players found matching %s" % user)
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['banall']['syntax'])
            ...
            # !bots on/off
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if arg == "on":
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], "^7Bot support: ^2On")
                        self.game.rcon_tell(
                            sar['player_num'], "^3Map cycle may be required to enable bot support")
                    elif arg == "off":
                        self.game.send_rcon('bot_enable 0')
                        self.game.send_rcon('kick allbots')
                        self.game.rcon_tell(
                            sar['player_num'], "^7Bot support: ^1Off")
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['bots']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['bots']['syntax'])
            ...
            # clear - clear all player warnings - !clear [<player>]
                ...
                if line.split(sar['command'])[1]:
                    ...
                    for player in self.game.players.itervalues():
                        player.clear_warning()
                    self.game.rcon_say(
                        "^1All player warnings and team kills cleared")
            ...
            # map - load given map
                ...
                if line.split(sar['command'])[1]:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['map']['syntax'])
            ...
            # moon - activate Moon mode (low gravity)
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if arg == "off":
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], "^7Moon mode: ^1Off")
                    elif arg == "on":
                        self.game.send_rcon('g_gravity 100')
                        self.game.rcon_tell(
                            sar['player_num'], "^7Moon mode: ^2On")
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['moon']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['moon']['syntax'])
            ...
            # !setgravity <value> - set the gravity value. default = 800 (less means less gravity)
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if arg.isdigit():
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['setgravity']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['setgravity']['syntax'])
                ...
                if self.urt_modversion >= 43:
                    if line.split(sar['command'])[1]:
                        ...
                        if arg == "off":
                            ...
                            self.game.rcon_tell(
                                sar['player_num'], "^7Instagib: ^1Off")
                            self.game.rcon_tell(
                                sar['player_num'], "^7Instagib changed for next map")
                        elif arg == "on":
                            self.game.send_rcon('g_instagib 1')
                            self.game.rcon_tell(
                                sar['player_num'], "^7Instagib: ^2On")
                            self.game.rcon_tell(
                                sar['player_num'], "^7Instagib changed for next map")
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], COMMANDS['instagib']['syntax'])
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['instagib']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7The command ^3!instagib ^7is not supported")
            ...
            # setnextmap - set the given map as nextmap
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], "^7Next Map set to: ^3%s" % nextmap)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['setnextmap']['syntax'])
            ...
            # rebuild - sync up all available maps
                ...
                self.game.rcon_tell(sar['player_num'], "^7Map count: [^2%s^7] maps found" % len(self.game.get_all_maps()))
                # set current and next map
                self.game.set_current_map()
                self.game.rcon_tell(sar['player_num'], self.get_nextmap())
                settings_bot = CONFIG.get('server', 'settings_bot')
                download_folder = CONFIG.get('server', 'download_folder')
                map_files = os.listdir(download_folder)
                mapcycle_entries = []
                for file_name in map_files:
                    if file_name.lower().endswith(".pk3") and not file_name.lower().startswith("z"):
                        map_name = os.path.splitext(file_name)[0]
                        mapcycle_entries.append(map_name)

                # Read the existing settings.conf file
                config = ConfigParser.ConfigParser()
                config.read(settings_bot)

                # Update the mapcycle section with the new map list
                config.set('mapcycle', 'small_cycle', ', '.join(mapcycle_entries))

                # Save the updated settings.conf file
                with open(settings_bot, 'w') as config_file:
                    config.write(config_file)

                self.game.rcon_tell(sar['player_num'], "^7Mapcycle updated with %s maps." % len(mapcycle_entries))
            ...
            # exec - execute given config file
                ...
                if line.split(sar['command'])[1]:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['exec']['syntax'])
            ...
            # !gear - set allowed weapons
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if "all" in arg:
                        ...
                        self.game.send_rcon(
                            'g_gear "%s"' % 'FGHIJKLMNZacefghijklOQRSTUVWX' if self.urt_modversion > 41 else '63')
                        self.game.rcon_say("^7Gear: ^2Knife only")
                    elif "pistol" in arg:
                        self.game.send_rcon(
                            'g_gear "%s"' % 'HIJKLMNZacehijkOQ' if self.urt_modversion > 41 else '55')
                        self.game.rcon_say("^7Gear: ^2Pistols only")
                    elif "shotgun" in arg:
                        self.game.send_rcon(
                            'g_gear "%s"' % 'FGIJKLMNZacefghiklOQ' if self.urt_modversion > 41 else '59')
                        self.game.rcon_say("^7Gear: ^2Shotguns only")
                    elif "sniper" in arg:
                        self.game.send_rcon(
                            'g_gear "%s"' % 'FGHIJKLMacefghjklOQ' if self.urt_modversion > 41 else '61')
                        self.game.rcon_say("^7Gear: ^2Sniper rifles only")
                    elif "magnum" in arg and self.urt_modversion > 42:
                        self.game.send_rcon(
                            'g_gear FGHIJKLMNZacefghijkOQRSTUVWX')
                        ...
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['gear']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['gear']['syntax'])
                ...
                if self.urt_modversion > 41:
                    if line.split(sar['command'])[1]:
                        ...
                        if not found:
                            ...
                            if victim.get_admin_role() >= self.game.players[sar['player_num']].get_admin_role():
                                self.game.rcon_tell(
                                    sar['player_num'], "^3Insufficient privileges to kill an admin")
                            else:
                                self.game.send_rcon(
                                    "smite %d" % victim.get_player_num())
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['kill']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7The command ^3!kill ^7is not supported")
            ...
            # lastadmin - display the last disconnected admin
                ...
                msg = "^7Last disconnected admin: ^3%s [^2%d^3]" % (self.last_admin.get_name(
                ), self.last_admin.get_admin_role()) if self.last_admin else "^3No admin left the server yet"
                self.game.rcon_tell(sar['player_num'], msg)
            ...
            # lookup - search for player in database
                ...
                if line.split(sar['command'])[1]:
                    arg = line.split(sar['command'])[1].strip()
                    curs.execute(
                        "SELECT `id`,`name`,`time_joined` FROM `player` WHERE `name` like '%{}%' ORDER BY `time_joined` DESC LIMIT 8".format(arg))
                    result = curs.fetchall()
                    for row in result:
                        self.game.rcon_tell(sar['player_num'], "^7[^2@%s^7] %s ^7[^1%s^7]" % (
                            str(row[0]), str(row[1]), str(row[2])), False)
                    if not result:
                        self.game.rcon_tell(
                            sar['player_num'], "^3No Player found matching %s" % arg)
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['lookup']['syntax'])
                        ...
                        if not found:
                            ...
                            if sar['player_num'] == victim.get_player_num():
                                ...
                                victim.ban(duration=630720000, reason=reason,
                                           admin=self.game.players[sar['player_num']].get_name())
                                self.game.rcon_say("^2%s ^1banned permanently ^7by %s: ^4%s" % (
                                    victim.get_name(), self.game.players[sar['player_num']].get_name(), reason))
                                self.game.kick_player(victim.get_player_num())
                                # add IP address to bot-banlist.txt
                                with open(os.path.join(HOME, 'bot-banlist.txt'), 'a') as banlist:
                                    banlist.write("%s:-1   // %s    banned on  %s, reason : %s\n" % (victim.get_ip_address(
                                    ), victim.get_name(), time.strftime("%d/%m/%Y (%H:%M)", time.localtime(time.time())), reason))
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], "^7You need to enter a reason: ^3!permban <name> <reason>")
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['permban']['syntax'])
            ...
            # makereg - make a player a regular (Level 2) user
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if victim.get_registered_user():
                            if victim.get_admin_role() < 2:
                                ...
                                self.game.rcon_tell(
                                    sar['player_num'], "^1%s ^7put in group Regular" % victim.get_name())
                            else:
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s is already in a higher level group" % victim.get_name())
                        else:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['makereg']['syntax'])
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if victim.get_admin_role() == 2:
                            ...
                            self.game.rcon_tell(
                                sar['player_num'], "^1%s ^7put in group User" % victim.get_name())
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^3%s is not in the regular group" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['unreg']['syntax'])
                    ...
                    if len(arg) > 1:
                        ...
                        if not found:
                            ...
                            if right == "user" and victim.get_admin_role() < 80:
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s put in group ^7User" % victim.get_name())
                                new_role = 1
                            elif "reg" in right and victim.get_admin_role() < 80:
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s put in group ^7Regular" % victim.get_name())
                                new_role = 2
                            elif "mod" in right and victim.get_admin_role() < 80:
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s added as ^7Moderator" % victim.get_name())
                                self.game.rcon_tell(
                                    victim.get_player_num(), "^3You are added as ^7Moderator")
                                new_role = 20
                            elif right == "admin" and victim.get_admin_role() < 80:
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s added as ^7Admin" % victim.get_name())
                                self.game.rcon_tell(
                                    victim.get_player_num(), "^3You are added as ^7Admin")
                                new_role = 40
                            elif "fulladmin" in right and victim.get_admin_role() < 80:
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s added as ^7Full Admin" % victim.get_name())
                                self.game.rcon_tell(
                                    victim.get_player_num(), "^3You are added as ^7Full Admin")
                                new_role = 60
                            # Note: senioradmin level can only be set by head admin or super admin
                                ...
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s added as ^6Senior Admin" % victim.get_name())
                                self.game.rcon_tell(victim.get_player_num(
                                ), "^3You are added as ^6Senior Admin")
                                new_role = 80
                            # Note: superadmin level can only be set by head admin
                                ...
                                self.game.rcon_tell(
                                    sar['player_num'], "^3%s added as ^2Super Admin" % victim.get_name())
                                self.game.rcon_tell(
                                    victim.get_player_num(), "^3You are added as ^2Super Admin")
                                new_role = 90
                            else:
                                self.game.rcon_tell(sar['player_num'], "^3Sorry, you cannot put %s in group <%s>" % (
                                    victim.get_name(), right))
                            victim.update_db_admin_role(role=new_role)
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['putgroup']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['putgroup']['syntax'])
            ...
            # banlist - display the last active 10 bans
                ...
                curs.execute("SELECT `id`,`name` FROM `ban_list` WHERE `expires` > '{}' ORDER BY `timestamp` DESC LIMIT 10".format(
                    time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))))
                result = curs.fetchall()
                banlist = ['^7[^2@%s^7] %s' %
                           (row[0], row[1]) for row in result]
                msg = 'Currently no one is banned' if not banlist else str(
                    ", ".join(banlist))
            ...
            # lastbans - list the last 4 bans
                ...
                curs.execute(
                    "SELECT `id`,`name`,`expires` FROM `ban_list` ORDER BY `timestamp` DESC LIMIT 4")
                result = curs.fetchall()
                lastbanlist = [
                    '^3[^2@%s^3] ^7%s ^3(^1%s^3)' % (row[0], row[1], row[2]) for row in result]
                if result:
                    for item in lastbanlist:
                        self.game.rcon_tell(sar['player_num'], str(item))
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7Currently no one is banned")
            ...
            # unban - unban a player from the database via ID
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if arg.isdigit():
                        curs.execute(
                            "SELECT `guid`,`name`,`ip_address` FROM `ban_list` WHERE `id` = {}".format(int(arg)))
                        result = curs.fetchone()
                        if result:
                            guid = result[0]
                            name = str(result[1])
                            ...
                            curs.execute(
                                "DELETE FROM `ban_list` WHERE `id` = {}".format(int(arg)))
                            conn.commit()
                            self.game.rcon_tell(
                                sar['player_num'], "^7Player ^2%s ^7unbanned" % name)
                            curs.execute(
                                "DELETE FROM `ban_list` WHERE `guid` = '{}' OR ip_address = '{}'".format(guid, ip_addr))
                            conn.commit()
                            self.game.rcon_tell(
                                sar['player_num'], "^7Try to remove duplicates of [^1%s^7]" % ip_addr)
                        else:
                            self.game.rcon_tell(
                                sar['player_num'], "^7Invalid ID, no Player found")
                    else:
                        self.game.rcon_tell(
                            sar['player_num'], COMMANDS['unban']['syntax'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['unban']['syntax'])
...
# head admin level 100 or super admin level 90
            # password - set private server password
                ...
                if line.split(sar['command'])[1]:
                    ...
                    self.game.rcon_tell(
                        sar['player_num'], "^7Password set to '%s' - Server is private" % arg)
                else:
                    self.game.send_rcon('g_password ""')
                    self.game.rcon_tell(
                        sar['player_num'], "^7Password removed - Server is public")
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if not found:
                        ...
                        if (1 < victim.get_admin_role() < COMMANDS['ungroup']['level'] or self.game.players[sar['player_num']].get_admin_role() == 100) and victim.get_player_num() != sar['player_num']:
                            ...
                            self.game.rcon_tell(
                                sar['player_num'], "^3Sorry, you cannot put %s in group User" % victim.get_name())
                else:
                    self.game.rcon_tell(
                        sar['player_num'], COMMANDS['ungroup']['syntax'])
            ...
            # switch to gametype
                ...
                self.game.rcon_tell(sar['player_num'],
                                    "^7Game Mode: ^2Free For All")
                self.game.rcon_tell(sar['player_num'],
                                    "^7Mode changed for next map")
            ...
            # iamgod - register user as Head Admin
                ...
                if self.iamgod:
                    if not self.game.players[sar['player_num']].get_registered_user():
                        ...
                        self.game.players[sar['player_num']
                                          ].register_user_db(role=100)
                    else:
                        self.game.players[sar['player_num']
                                          ].update_db_admin_role(role=100)
                    self.iamgod = False
                    self.game.rcon_tell(
                        sar['player_num'], "^7You are registered as ^6Head Admin")
...
# unknown command
                ...
                if sar['command'].lstrip('!') in self.superadmin_cmds:
                    self.game.rcon_tell(
                        sar['player_num'], "^7Insufficient privileges to use command ^3%s" % sar['command'])
                else:
                    self.game.rcon_tell(
                        sar['player_num'], "^7Unknown command ^3%s" % sar['command'])
...
# bad words
    ...
    def kick_high_warns(self, player, reason, text):
        if player.get_warning() > 3:
            self.game.rcon_say("^2%s ^7was kicked, %s" %
                               (player.get_name(), reason))
            self.game.kick_player(player.get_player_num(), reason=reason)
        else:
            self.game.rcon_tell(player.get_player_num(
            ), "^1WARNING ^7[^3%d^7]: %s" % (player.get_warning(), text))
            if player.get_warning() == 3:
                self.game.rcon_say(
                    "^1ALERT: ^2%s ^7auto-kick from warnings if not cleared" % player.get_name())
    ...
    def get_admins_online(self):
        ...
        """
        liste = "%s" % ", ".join(["^3%s [^2%d^3]" % (player.get_name(), player.get_admin_role(
        )) for player in self.game.players.itervalues() if player.get_admin_role() >= 20])
    ...
    def get_admin_count(self):
        ...
        """
        counter = sum(1 for player in self.game.players.itervalues()
    ...
    def handle_flag(self, line):
        ...
        with self.players_lock:
            ...
            if action == '1:':
                player.return_flag()
                logger.debug("Player %d %s returned the flag",
                             player_num, player_name)
            elif action == '2:':
                player.capture_flag()
                cap_count = player.get_flags_captured()
                self.game.send_rcon("^7%s has captured ^2%s ^7flag%s" % (
                    player_name, cap_count, 's' if cap_count > 1 else ''))
                logger.debug("Player %d %s captured the flag",
                             player_num, player_name)
    ...
    def handle_bomb(self, line):
        ...
        with self.players_lock:
            tmp = line.split(
                "is") if "Bombholder" in line else line.split("by")
            action = tmp[0].strip()
            ...
            if action == 'Bomb was defused':
                ...
                self.game.send_rcon(
                    "^7The ^2BOMB ^7has been defused by ^2%s^7!" % name)
                ...
                if self.kill_survived_opponents and self.urt_modversion > 41:
                    for player in self.game.players.itervalues():
                        if player.get_team() == 1 and player.get_alive():
                            self.game.send_rcon("smite %d" %
                                                player.get_player_num())
            elif action == 'Bomb was planted':
                player.planted_bomb()
                logger.debug("Player %d %s planted the bomb", player_num, name)
                self.game.send_rcon("^7The ^1BOMB ^7has been planted by ^1%s^7! ^2%s ^7seconds to detonation." % (
                    name, self.explode_time))
                if self.spam_bomb_planted_msg:
                    self.game.rcon_bigtext(
                        "^1The ^7BOMB ^1has been planted by ^7%s^1!" % name)
                    self.game.rcon_bigtext(
                        "^7The ^1BOMB ^7has been planted by ^1%s^7!" % name)
                    self.game.rcon_bigtext(
                        "^1The ^7BOMB ^1has been planted by ^7%s^1!" % name)
                    self.game.rcon_bigtext(
                        "^7The ^1BOMB ^7has been planted by ^1%s^7!" % name)
                ...
                for mate in self.game.players.itervalues():
                    if mate.get_team() == 1 and mate.get_alive() and mate != player:
                        self.game.rcon_tell(
                            mate.get_player_num(), "^7The ^1BOMB ^7is loose!")
            elif action == 'Bomb has been collected':
                player.is_bombholder()
                for mate in self.game.players.itervalues():
                    if mate.get_team() == 1 and mate.get_alive() and mate != player:
                        self.game.rcon_tell(mate.get_player_num(
                        ), "^7Help ^1%s ^7to plant the ^1BOMB" % name)
    ...
    def handle_teams_ts_mode(self, line):
        ...
        self.game.send_rcon("%s%s ^7team wins" % (
            '^1' if line == 'Red' else '^4', line) if 'Draw' not in line else "^7Draw")
        self.autobalancer()
    ...
    def handle_team_balance(self):
        ...
        with self.players_lock:
            game_data = self.game.get_gamestats()
            self.game.rcon_say("^7Red: ^1%s ^7- Blue: ^4%s ^7- Spectator: ^3%s" % (
                game_data[Player.teams[1]], game_data[Player.teams[2]], game_data[Player.teams[3]]))
            if (abs(game_data[Player.teams[1]] - game_data[Player.teams[2]])) > 1:
                if self.allow_cmd_teams:
                    ...
                    if self.ts_gametype or self.bomb_gametype or self.freeze_gametype:
                        ...
                        self.game.rcon_say(
                            "^7Teams will be balanced at the end of this round!")
            else:
    ...
    def handle_awards(self):
        ...
        with self.players_lock:
            for player in self.game.players.itervalues():
                ...
                if player.get_team() != 3:
                    if self.freeze_gametype:
                        self.game.rcon_tell(player_num, "^7Stats %s: ^7F ^2%d ^7T ^3%d ^7HS ^1%d ^7TK ^1%d" % (
                            player_name, player.get_freeze(), player.get_thawout(), player.get_headshots(), player.get_team_kill_count()))
                    else:
                        self.game.rcon_tell(player_num, "^7Stats %s: ^7K ^2%d ^7D ^3%d ^7HS ^1%d ^7TK ^1%d" % (
                            player_name, player_kills, player.get_deaths(), player_headshots, player.get_team_kill_count()))
            ...
            if most_planted > 1 or most_defused > 1:
                self.game.rcon_say("^2Top Objectives: ^7%s [^1%s^7]" % (
                    (planted_by, most_planted) if most_planted > most_defused else (defused_by, most_defused)))
            # CTF statistics
            if most_flags > 1:
                self.game.rcon_say(
                    "^2Top Objectives: ^7%s [^1%s^7]" % (flagrunner, most_flags))

            # HE grenade kills
            if most_he_kills > 1:
                self.game.rcon_say(
                    "^2Most HE grenade kills: ^7%s (^1%d ^7HE kills)" % (nader, most_he_kills))

            if most_knife_kills > 1:
                self.game.rcon_say("^2Most knife kills: ^7%s (^1%d ^7kills)" % (
                    knifer, most_knife_kills))

            # CTF statistics
            if fastest_cap < 999:
                self.game.rcon_say(
                    "^2Fastest cap: ^7%s (^1%s ^7sec)" % (fastrunner, fastest_cap))
            if most_flag_returns > 1:
                self.game.rcon_say("^2Best defender: ^7%s (^1%d ^7flag returns)" % (
                    defender, most_flag_returns))
...
class Player(object):
    ...
    roles = {0: "Guest", 1: "User", 2: "Regular", 20: "Moderator", 40: "Admin",
             60: "Full Admin", 80: "Senior Admin", 90: "Super Admin", 100: "Head Admin"}
    ...
    def __init__(self, player_num, ip_address, guid, name, auth=''):
        ...
        now = time.strftime("%Y-%m-%d %H:%M:%S",
                            time.localtime(self.time_joined))
        curs.execute("SELECT `id`,`reason` FROM `ban_list` WHERE `guid` = '{}' AND `expires` > '{}'".format(
            self.guid, now))
        result = curs.fetchone()
        if result:
            ...
            curs.execute("SELECT `id`,`reason` FROM `ban_list` WHERE `ip_address` = '{}' AND `expires` > '{}'".format(
                self.address, now))
            result = curs.fetchone()
            if result:
                self.ban_id = result[0]
    ...
    def ban(self, duration=900, reason='tk', admin=None):
        ...
        try:
            expire_date = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(time.time() + duration))
        except ValueError:
            expire_date = time.strftime(
                "%Y-%m-%d %H:%M:%S", time.localtime(2147483647))
        timestamp = time.strftime(
            "%Y-%m-%d %H:%M:%S", time.localtime(time.time()))
        curs.execute(
            "SELECT `expires` FROM `ban_list` WHERE `guid` = '{}'".format(self.guid))
        result = curs.fetchone()
        if result:
            if result[0] < expire_date:
                # update already existing ban
                curs.execute("UPDATE `ban_list` SET `ip_address` = '{}',`expires` = '{}',`reason` = '{}' WHERE `guid` = '{}'".format(
                    self.address, expire_date, reason, self.guid))
                conn.commit()
                return True
            # update IP address of existing ban
            curs.execute("UPDATE `ban_list` SET `ip_address` = '{}' WHERE `guid` = '{}'".format(
                self.address, self.guid))
            conn.commit()
            return False
        # create new ban
        curs.execute("INSERT INTO `ban_list` (`id`,`guid`,`name`,`ip_address`,`expires`,`timestamp`,`reason`) VALUES ({},'{}','{}','{}','{}','{}','{}')".format(
            self.player_id, self.guid, self.name, self.address, expire_date, timestamp, reason))
        conn.commit()
        return True
    ...
    def add_ban_point(self, point_type, duration):
        ...
        curs.execute("INSERT INTO `ban_points` (`guid`,`point_type`,`expires`) VALUES ('{}','{}','{}')".format(
            self.guid, point_type, expire_date))
        conn.commit()
        # check amount of ban_points
        curs.execute("SELECT COUNT(*) FROM `ban_points` WHERE `guid` = '{}' AND `expires` > '{}'".format(
            self.guid, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))))
    ...
    def save_info(self):
        if self.registered_user:
            ratio = round(float(self.db_kills) / float(self.db_deaths),
                          2) if self.db_deaths > 0 else 1.0
            curs.execute("UPDATE `xlrstats` SET `kills` = {},`deaths` = {},`headshots` = {},`team_kills` = {},`team_death` = {},`max_kill_streak` = {},`suicides` = {},`rounds` = `rounds` + 1,`ratio` = {} WHERE `guid` = '{}'".format(
                self.db_kills, self.db_deaths, self.db_head_shots, self.db_tk_count, self.db_team_death, self.db_killing_streak, self.db_suicide, ratio, self.guid))
            conn.commit()
    ...
    def check_database(self):
        ...
        # check player table
        curs.execute(
            "SELECT COUNT(*) FROM `player` WHERE `guid` = '{}'".format(self.guid))
        if int(curs.fetchone()[0]) == 0:
            # add new player to database
            curs.execute("INSERT INTO `player` (`guid`,`name`,`ip_address`,`time_joined`,`aliases`) VALUES ('{}','{}','{}','{}','{}')".format(
                self.guid, self.name, self.address, now, self.name))
            conn.commit()
            self.aliases.append(self.name)
            self.first_time = True
        else:
            # update name, IP address and last join date
            curs.execute("UPDATE `player` SET `name` = '{}',`ip_address` = '{}',`time_joined` = '{}' WHERE `guid` = '{}'".format(
                self.name, self.address, now, self.guid))
            conn.commit()
            # get known aliases
            curs.execute(
                "SELECT `aliases` FROM `player` WHERE `guid` = '{}'".format(self.guid))
            result = curs.fetchone()
            ...
            if self.name not in self.aliases and len(self.aliases) < 15:
                ...
                curs.execute("UPDATE `player` SET `aliases` = '{}' WHERE `guid` = '{}'".format(
                    alias_string, self.guid))
                conn.commit()
        # get player-id
        curs.execute(
            "SELECT `id` FROM `player` WHERE `guid` = '{}'".format(self.guid))
        self.player_id = curs.fetchone()[0]
        # check XLRSTATS table
        curs.execute(
            "SELECT COUNT(*) FROM `xlrstats` WHERE `guid` = '{}'".format(self.guid))
        if int(curs.fetchone()[0]) == 0:
            self.registered_user = False
        else:
            ...
            curs.execute(
                "UPDATE `xlrstats` SET `name` = '{}',`last_played` = '{}',`num_played` = `num_played` + 1 WHERE `guid` = '{}'".format(self.name, now, self.guid))
            conn.commit()
    ...
    def define_offline_player(self, player_id):
        ...
        if int(curs.fetchone()[0]) == 0:
            ...
            curs.execute(
                "SELECT `last_played`,`admin_role` FROM `xlrstats` WHERE `guid` = '{}'".format(self.guid))
            result = curs.fetchone()
            self.last_visit = result[0]
    ...
    def register_user_db(self, role=1):
        if not self.registered_user:
            ...
            curs.execute("INSERT INTO `xlrstats` (`guid`,`name`,`ip_address`,`first_seen`,`last_played`,`num_played`,`admin_role`) VALUES ('{}','{}','{}','{}','{}',1,{})".format(
                self.guid, self.name, self.address, now, now, role))
            conn.commit()
            self.registered_user = True
    ...
    def update_db_admin_role(self, role):
        curs.execute("UPDATE `xlrstats` SET `admin_role` = {} WHERE `guid` = '{}'".format(
            role, self.guid))
        conn.commit()
    ...
    def clear_warning(self):
        ...
        self.last_warn_time = 0
        # clear ban_points
        curs.execute("DELETE FROM `ban_points` WHERE `guid` = '{}' and `expires` > '{}'".format(
            self.guid, time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(time.time()))))
        conn.commit()
...
class Game(object):
    ...
    def __init__(self, urt_modversion):
        ...
        self.quake = PyQuake3("%s:%s" % (CONFIG.get('server', 'server_ip'), CONFIG.get(
            'server', 'server_port')), CONFIG.get('server', 'rcon_password'))
        self.queue = Queue()
        ...
        # dynamic mapcycle
        self.dynamic_mapcycle = CONFIG.getboolean('mapcycle', 'dynamic_mapcycle') if CONFIG.has_option(
            'mapcycle', 'dynamic_mapcycle') else False
        if self.dynamic_mapcycle:
            self.switch_count = CONFIG.getint('mapcycle', 'switch_count') if CONFIG.has_option(
                'mapcycle', 'switch_count') else 4
            self.big_cycle = filter(None, CONFIG.get('mapcycle', 'big_cycle').replace(
                ' ', '').split(',')) if CONFIG.has_option('mapcycle', 'big_cycle') else []
            self.small_cycle = filter(None, CONFIG.get('mapcycle', 'small_cycle').replace(
                ' ', '').split(',')) if CONFIG.has_option('mapcycle', 'small_cycle') else []
        ...
        logger.info(
            "Spunky Bot is running until you are closing this session or pressing CTRL + C to abort this process.")
        logger.info(
            "*** Note: Use the provided initscript to run Spunky Bot as daemon ***")
    ...
    def get_cvar(self, value):
        ...
        if self.live:
            with self.rcon_lock:
                try:
                    ret_val = self.quake.rcon(value)[1].split(
                        ':', 1)[1].split('^7')[0].lstrip('"')
                except IndexError:
                    ret_val = None
                time.sleep(RCON_DELAY)
                return ret_val
    ...
    def get_mapcycle_path(self):
        ...
        try:
            ...
            mc_home_path = os.path.join(
                fs_homepath, fs_game, mapcycle_file) if fs_homepath else ""
            mc_base_path = os.path.join(
                fs_basepath, fs_game, mapcycle_file) if fs_basepath else ""
        except TypeError:
            raise Exception(
                'Server did not respond to mapcycle path request, please restart the Bot')
        mapcycle_path = mc_home_path if os.path.isfile(
            mc_home_path) else mc_base_path if os.path.isfile(mc_base_path) else None
        if mapcycle_path:
    ...
    def kick_player(self, player_num, reason=''):
        ...
        logger.debug("%s kicked%s%s", player_num,
    ...
    def go_live(self):
        ...
        self.rcon_say(
            "^7Powered by ^8[Spunky Bot %s] ^1[www.spunkybot.de]" % __version__)
        logger.info("Mapcycle: %s", ', '.join(self.maplist))
        logger.info("*** Live tracking: Current map: %s / Next map: %s ***",
        ...
        if self.get_cvar('g_logsync') == '0':
            logger.error(
                "Server CVAR 'g_logsync' is set to '0' but must be '1' to support interaction with the bot")
    ...
    def set_current_map(self):
        ...
        if self.dynamic_mapcycle:
            self.maplist = filter(None, (self.small_cycle if self.get_number_players(
            ) < self.switch_count else self.big_cycle))
            logger.debug("Players online: %s / Mapcycle: %s",
                         self.get_number_players(), self.maplist)
        ...
        if self.maplist:
            if self.mapname in self.maplist:
                if self.maplist.index(self.mapname) < (len(self.maplist) - 1):
                    self.next_mapname = self.maplist[self.maplist.index(
                        self.mapname) + 1]
                else:
        ...
        logger.debug("Current map: %s / Next map: %s",
                     self.mapname, self.next_mapname)
        ...
        try:
            ...
            while True:
                ...
                if count >= 2:
                    ...
                    all_maps += ret_val
            all_maps_list = list(set([maps.replace(
                "/", "").replace(".bsp", "").lower() for maps in all_maps if maps.startswith("/")]))
            all_maps_list.sort()
            if all_maps_list:
    ...
    def get_gamestats(self):
        ...
        """
        game_data = {Player.teams[1]: 0,
                     Player.teams[2]: 0, Player.teams[3]: -1}
    ...
    def balance_teams(self, game_data):
        ...
        num_ptm = math.floor(
            (game_data[Player.teams[team1]] - game_data[Player.teams[team2]]) / 2)
        player_list = [player for player in self.players.itervalues(
        ) if player.get_team() == team1 and not player.get_team_lock()]
        player_list.sort(cmp=lambda player1, player2: cmp(
            player2.get_time_joined(), player1.get_time_joined()))
...
if __name__ == "__main__":
    ...
    verbose = CONFIG.getboolean('bot', 'verbose') if CONFIG.has_option(
        'bot', 'verbose') else False
    # logging format
    ...
    # devel.log file
    devel_log = logging.handlers.RotatingFileHandler(filename=os.path.join(
        HOME, 'devel.log'), maxBytes=2097152, backupCount=1, encoding='utf8')
