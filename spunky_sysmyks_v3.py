...
class LogParser(object):
    ...
    def parse_line(self, string):
        ...
        option = {'InitGame': self.new_game, 'Warmup': self.handle_warmup, 'InitRound': self.handle_initround,
                  ...
                  'ClientJumpRunStopped': self.handle_ClientJumpRunStopped}
        try:
            action = tmp[0].strip()
            if action in option:
                option[action](line)
    ...
    def handle_ClientJumpRunStopped(self, line):
        """
        handle ClientJumpRunStopped
        """
        # Utilisation des expressions régulières pour extraire les valeurs
        matches = re.findall(r'\d+', line)
        # Assignation des valeurs extraites aux variables spécifiées
        player_num = int(matches[0])
        ...
        # Création de la chaîne de message formatée
        msg = "^7Map : ^4{} ^7player : ^4{} ^7way: ^4{} ^7time: ^4{}^7:^4{}^7.^4{}^7".format(mapname, player_name, way, minutes, seconds, milliseconds)
        # Envoi du message à l'aide de self.game.send_rcon()
        self.game.rcon_bigtext("^7Your time : %s" % msg)
    ...
    def handle_begin(self, line):
        ...
        with self.players_lock:
            ...
            if player.get_registered_user() and player.get_welcome_msg():
                ...
                self.game.rcon_tell(player_num, "^7Welcome back %s, you are player ^3#%s^7." % (player_name, player_id))
                player.disable_welcome_msg()
            ...
            if sar['command'] == '!mapstats':
                ...
                if line.split(sar['command'])[1]:
                    ...
                    if self.game.players[sar['player_num']].get_admin_role() < 20:
                        self.game.rcon_tell(sar['player_num'], "^7Jumper commands: ^3%s" % ', ^3'.join(self.clean_cmd_list(self.user_cmds)))
